    ...
    public class EnquetePage : ContentPage
    {
        ...
        private async Task CarregaEnquetesInteresse()
        {
            try
            {
                ...
                foreach (var enquete in this.model.Enquetes.Where(x => (x.AtivaNoFront && (x.Status == EnumStatusEnquete.Ativa ||
                    x.Status == EnumStatusEnquete.Publicada))).Distinct())
                {
                ...
                var enquetesAgrupadas = from e in this.model.Enquetes.Where(x => (x.AtivaNoFront && (x.Status == EnumStatusEnquete.Ativa ||
                                                            x.Status == EnumStatusEnquete.Publicada))).Distinct()
                                                    group e by e.Categoria.Nome into g
                                                    select new
                {
